...
import { getOrSetDataWithCache, setDataToCache, getDataFromCache} from '@/lib/cache/cache_manager'
...
export async function getGlobalData({
  ...
  console.log(`>>>>>>>>>>>>>>>>>>>>>>>------ getGlobalData   : ${from} -----------`);
  ...
  try {
    for (let index = 0; index < siteIds.length; index++) {
      ...
      if (index === 0 || locale === prefix) {
        key =  `site-page-${id}`;
        const finaldata = await getDataFromCache(key)
        if (finaldata) {
          console.log(`[PID: ${process.pid}] **********use cache***********`,Object.keys(finaldata), finaldata.allPages?.length);
          return deepClone(finaldata);
        }
  ...
  data =  handleDataBeforeReturn(data)
  // 缓存
  await setDataToCache(key, data, BLOG.SITE_CACHE_TTL)
...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  ...
  if (cacheGlobalInfo && pageIds.length === cacheGlobalInfo.pageIds?.length) {
    if (cacheGlobalInfo) {
      console.log(' use site-info-cache from cache :', pageId, from);
    }
    return {...cacheGlobalInfo,
      collection,
      collectionQuery,
      collectionId,
      collectionView,
  ...
  console.log(`[PID: ${process.pid}] ---- no use cache-1---${from}-${pageId}`);
  ...
  console.log(`[PID: ${process.pid}] **********before set cache***********`, siteInfoCache?.allPages?.length);
