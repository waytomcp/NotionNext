...
import { getOrSetDataWithCache,getOrSetDataWithCustomCache,setDataToCache,getDataFromCache} from '@/lib/cache/cache_manager'
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  return await getOrSetDataWithCache(
    ...
    async (pageId, from) => {
      ...
      console.log(`>>>>>>>>*************--- getSiteDataByPageId:pageid:${pageId}  from: ${from} -----------`);
...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  ...
  console.log('**********pageIds:*********************len:', 'pageIds:', pageIds.length, 'blockIdsNeedFetch:', blockIdsNeedFetch.length);
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    if (!value) {
      console.error('pageId abase value is null', id)
    }
    const properties =
    ...
    if (properties) {
      if (properties.status === 'Published') {
        collectionData.push(properties);
      } else {
        //console.log('文章状态不是Published，不显示:', properties.title)
      }
    }
  }

  console.log ("-------------collectionProperties:", collectionData.length)
  const cacheGlobalInfo = await getDataFromCache('site-info-cache')
  console.log('cacheGlobalInfo:', cacheGlobalInfo?.tagOptions?.length, cacheGlobalInfo?.categoryOptions?.length, cacheGlobalInfo?.customMenu?.length);
  // if (cacheGlobalInfo) {
  //   console.log("get cacheGlobalInfo is not null:", cacheGlobalInfo)
  // } else {
  //   console.log("get cacheGlobalInfo:", cacheGlobalInfo) 
  // }


  // 站点配置优先读取配置表格，否则读取blog.config.js 文件
  let NOTION_CONFIG = cacheGlobalInfo?.NOTION_CONFIG || (await getConfigMapFromConfigPage(collectionData)) || {}
  // 处理每一条数据的字段
  ...
  // 站点基础信息
  const siteInfo = cacheGlobalInfo?.siteInfo || getSiteInfo({ collection, block, NOTION_CONFIG })
  ...
  // 所有分类
  const categoryOptions = cacheGlobalInfo?.categoryOptions || getAllCategories({
    allPages,
    categoryOptions: getCategoryOptions(schema)
  })

  // 所有标签
  const tagOptions = cacheGlobalInfo?.tagOptions || getAllTags({
    allPages,
    sliceCount: 20,
  ...
  // 旧的菜单
  const customNav = cacheGlobalInfo?.customNav || await getCustomNav({
    allPages: collectionData.filter(
      post => post?.type === 'Page' && post.status === 'Published'
    )
  })
  // 新的菜单
  const customMenu = cacheGlobalInfo?.customMenu || await getCustomMenu({ collectionData, NOTION_CONFIG })
  const latestPosts = getLatestPosts({ allPages, from, latestPostCount: 6 })
  const allNavPages = getNavPages({ allPages })

  //将siteInfo,tagOptions,categoryOptions,customMenu 放到一个对象中进行缓存，缓存时间为SITE_CACHE_TTL
  if (!cacheGlobalInfo) {
    const siteInfoCache = {
      NOTION_CONFIG,
      siteInfo,
    ...
    await setDataToCache('site-info-cache', siteInfoCache, BLOG.SITE_CACHE_TTL)
    console.log('缓存site-info-cache', siteInfoCache.tagOptions.length, siteInfoCache.categoryOptions.length, siteInfoCache.customMenu.length, BLOG.SITE_CACHE_TTL)
  } else {
    console.log('********* get cacheGlobalInfo is not null ******************:', cacheGlobalInfo.tagOptions.length,)
  }
