...
export async function processPostData(props, from) {
  // 文章内容加载
  console.log('************** processPostData**************:', props.post.id, props.post.title)
  if (!props?.post?.blockMap) {
    props.post.blockMap = await getPage(props.post.id, from)
  }

  if (props.post?.blockMap?.block) {
    // 目录默认加载
    console.log('************** processPostData**************Keys:', Object.keys(props.post.blockMap))
    ...
    console.log('************** processPostData******step4')
  }
  ...
  console.log('************** processPostData******step5')
  // 推荐关联文章处理
  const allPosts = props.allPages?.filter(
    page => page.type === 'Post' && page.status === 'Published'
  )
  console.log('************** processPostData******step6')
  ...
  console.log('************** processPostData******key ', Object.keys(props))
  delete props.allPages
  delete props.post.content
  delete props.post.blockMap.collection
  delete props.post.blockMap.collection_query
  delete props.post.blockMap.collection_view
  // delete props.notice
  delete props.categoryOptions
  delete props.tagOptions
  console.log('************** processPostData******step8')
  // props = {
  //   post: props.post,
  //   recommendPosts: props.recommendPosts,
  //   prev: props.prev,
  //   next: props.next,
